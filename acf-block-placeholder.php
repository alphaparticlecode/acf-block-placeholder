...
function my_acf_register_block_type_args( $args ){
    ...
    if( empty( $args['render_callback'] ) ) {
        $args['render_callback'] = 'maybe_generate_acf_block_placeholder';
    }
    
    return $args;
}

function maybe_generate_acf_block_placeholder( $block, $content = '', $is_preview = false, $post_id = 0 ) {

    if( $is_preview && is_block_empty( $block ) ) {
